...
class cmd(BaseCommand):
    ...
    async def execute(self, interaction, member:discord.member.Member = None):
        if member is None:
            fetch_data = await self.db.raw_exec_select("SELECT * FROM slashcox.fetch WHERE user=?", interaction.user)

        else:
            fetch_data = await self.db.raw_exec_select("SELECT * FROM slashcox.fetch WHERE user=?", member) 
